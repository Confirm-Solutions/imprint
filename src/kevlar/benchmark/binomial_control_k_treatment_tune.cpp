...
BENCHMARK_DEFINE_F(binomial_fixture, bench_tune)(benchmark::State& state) {
    ...
    auto rng_gen_f = [=](auto& gen, auto& rng) {
        ...
        rng = Eigen::MatrixXd::NullaryExpr(
            n_samples, grid_dim, [&](auto, auto) { return unif(gen); });
    };

    BinomialControlkTreatment<grid::Rectangular> model(grid_dim, ph2_size,
                                                       n_samples);
    ...
    for (auto _ : state) {
        ...
        try {
            auto thr = tune(n_sim, alpha, delta, grid_dim, grid_radius, p_1d,
                            p_endpt, thr_vec, rng_gen_f, model, 0, batch_size,
...
BENCHMARK_REGISTER_F(binomial_fixture, bench_tune)->ArgsProduct({{30}, {}});
