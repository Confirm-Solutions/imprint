...
int main()
{
    ...
    tune(100, alpha, [](){ return Eigen::VectorXd(); },
            [&](const auto&) {
                auto J = p.rows();
                auto d = p.cols();
                ...
                Eigen::MatrixXi n_action_arms(J, d); n_action_arms.setZero();
                Eigen::MatrixXi successes(J, d); successes.setZero();
                Eigen::MatrixXd beta_posterior(J, d); beta_posterior.setZero();
                Eigen::VectorXd alpha_posterior_buff(J * d); alpha_posterior_buff.setZero();
                Eigen::Map<Eigen::MatrixXd> alpha_posterior(alpha_posterior_buff.data(), J, d);

                thompson.sample(p, beta, alpha_prior, beta_prior, max_patients,
                                n_action_arms, successes,
                                alpha_posterior, beta_posterior);
                Eigen::VectorXd out = alpha_posterior.col(alpha_posterior.cols()-1);
                return out;
            });
