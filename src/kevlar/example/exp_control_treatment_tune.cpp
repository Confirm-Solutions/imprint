...
int main() {
    ...
    Eigen::VectorXd lmda_lower =
        grid_t::make_endpts(lmda_size, lower, upper).row(0);
    lmda_lower.array() = lmda_lower.array().exp();

    Eigen::VectorXd hzrd_rate =
        grid_t::make_grid(lmda_size, hzrd_lower, hzrd_upper);
    hzrd_rate.array() = hzrd_rate.array().exp();

    Eigen::VectorXd hzrd_rate_lower =
        grid_t::make_endpts(lmda_size, hzrd_lower, hzrd_upper).row(0);
    ...
    ExpControlkTreatment<grid::Rectangular> model(n_samples, censor_time,
                                                  lmda_1d, lmda_lower,
                                                  hzrd_rate, hzrd_rate_lower);

    try {
        auto thr = tune(model, n_sim, alpha, delta, grid_radius, thr_vec, 0,
