...
struct InterSum {
    ...
    InterSum() = default;
    InterSum(size_t n_models, size_t n_tiles, size_t n_params)
     ...
     *                              Assumes that the sequence of models
     * considered is ordered in ascending order, in the sense that if ith model
     * rejects then jth model rejects for all j >= i.
     */
        ...
        for (uint_t i = 0; i < n_gridpts; ++i) {
            ...
            if (gr_view.is_regular(i)) {
                if (unlikely(rej_len_[pos] != 0)) {
                    update_internal(pos,
                                    [&](uint_t k) { return state.grad(i, k); });
                }
                ++pos;
                continue;
            }
   ...
   private:
    template <class F>
    KEVLAR_STRONG_INLINE void update_internal(uint_t pos, F get_grad) {
        ...
        // nabla_eta A(m) is the gradient under the natural parameter eta of the
        // log-partition function for arm k evaluated at mean m.
        ...
        for (uint_t k = 0; k < n_params_; ++k, slice_offset += slice_size) {
            Eigen::Map<mat_type<value_t> > grad_k_cache(
                grad_sum_.data() + slice_offset, type_I_sum_.rows(),
                type_I_sum_.cols());
    ...
    mat_type<uint_t> type_I_sum_;  // Type I error sums.
                                   // type_I_sum_(i,j) = rejection accumulation
                                   // for model i at tile j.
    colvec_type<value_t>
        grad_sum_;     // gradient sums.
                       // grad_sum_(i,j,k) = partial deriv accumulation w.r.t.
                       // param k for model i at tile j.
    size_t n_params_;  // dimension of a gridpoint.

    /* Buffer needed in update for one-time allocation */
    colvec_type<uint_t>
        rej_len_;  // number of models that rejects for each gridpt
    colvec_type<value_t>
        grad_buff_;  // gradient vector buffer for general tile case
};
