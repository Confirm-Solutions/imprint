...
public:
                ...
                ValueType finalize_thr) const
    {
        ...
        const auto n_new_pts = ipow(2, d);    // number of new points if eps changes
        ...
        for (size_t j = 0; j < grid_range.n_gridpts(); ++j) 
        {
            ...
            bool any_N = false;  // true if any tile require increase in sim_size 
        ...
        for (size_t j = 0; j < grid_range_old.n_gridpts(); ++j) 
        {
            ...
            switch(actions[j]) 
            {
                ...
                case action_type::N_:
                {
                    ...
                    sim_sizes_new[new_j] = compute_new_sim_size(
                            sim_size_old_j, N_factor, N_max);
                    ++new_j;
                    break;
                }
