 ...
 *                      depending on if it is in the non-negative,
 *                      non-positive, or boundary orientation.
 *                      Otherwise, it will be set to orient_type::none.
 *
 * @return  true if max tile is on one side of sf.
 */
template <class TileType
        , class SurfaceType
        , class SaveOrientType
        , class IterType>
inline bool is_oriented_(
        const TileType& tile,
        ...
        IterType end)
{
    ...
    auto ori = (n_pos > 0) ? orient_type::non_neg :
...
inline bool is_oriented(
        const TileType& tile,
        const SurfaceType& sf) 
{
    return internal::is_oriented_(
            tile, sf, [](auto){});
}
...
{
    return internal::is_oriented_(
            tile, sf, [&](orient_type r) { reason = r; });
}
