...
struct traits<ExpControlkTreatment<ValueType, UIntType, GridRangeType> >
{
    ...
    using state_t = typename ExpControlkTreatment<value_t, uint_t, grid_range_t>::StateType;
};
...
public:
            ...
            for (size_t i = 0; i < gr_view.n_gridpts(); ++i) {
                ...
                if (gr_view.is_regular(i)) {
                    rej_len[pos] = (likely(gr_view.check_null(pos, 0))) ?
                        rej_len_internal(i, hzrd_rate_prev, do_logrank_update) : 0;
                    ++pos;
                    continue;
                }
                ...
                for (size_t t = 0; t < gr_view.n_tiles(i); ++t, ++pos) {
                    ...
                    if (!internal_called && is_null) {
                        rej = rej_len_internal(i, hzrd_rate_prev, do_logrank_update);
    ...
    private:
        ...
        size_t rej_len_internal(
                size_t i, 
                value_t& hzrd_rate_prev,
                bool& do_logrank_update) 
        {
            ...
            if (do_logrank_update) {
                ...
                int cr_idx = 0, tr_idx = 0, cs_idx = 0; // control, treatment, and cum_sum index
                ...
                while (cr_idx < exp_control.size() && tr_idx < exp_treatment.size()) 
                {
                    ...
                    auto N = N_j.sum();
                    auto E_1j = N_j[1] / N;
                    logrank_cum_sum_[cs_idx+1] = logrank_cum_sum_[cs_idx] + (O_1j - E_1j);
                    v_cum_sum_[cs_idx+1] = v_cum_sum_[cs_idx] + E_1j * (1-E_1j);
                ...
                logrank_cum_sum_.tail(tot-cs_idx).array() = logrank_cum_sum_[cs_idx];
    ...
    void set_grid_range(const grid_range_t& grid_range) 
    {
        ...
        pars_lower.array() = (grid_range.thetas() - grid_range.radii()).array().exp();
    }
