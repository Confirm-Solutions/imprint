    ...
    py::class_<eckt_t, ckt_t, mb_t>(m, "ExpControlkTreatment")
        ...
        .def(py::pickle(
            [](const eckt_t& p) {  // __getstate__
                ...
                return py::make_tuple(p.n_samples(), p.censor_time__(),
                                      p.thresholds__(), p.n_gridpts__(),
                                      p.buff__());
            },
            [](py::tuple t) {  // __setstate__
                ...
                eckt_t p(t[0].cast<size_t>(), t[1].cast<value_t>(),
                         t[2].cast<Eigen::Ref<const colvec_type<double>>>());
                auto n_gridpts = t[3].cast<std::decay_t<
                    decltype(std::declval<eckt_t>().n_gridpts__())>>();
                auto&& buff = t[4].cast<
