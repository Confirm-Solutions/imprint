...
template <class MSBType, class ISType>
void add_inter_sum(pybind11::module_& m) {
    ...
    py::class_<is_t>(m, "InterSum")
        ...
        .def("type_I_sum", py::overload_cast<>(&is_t::type_I_sum),
             py::return_value_policy::reference_internal)
        .def("type_I_sum_const",
             py::overload_cast<>(&is_t::type_I_sum, py::const_),
             py::return_value_policy::reference_internal)
        .def("grad_sum", py::overload_cast<>(&is_t::grad_sum),
             py::return_value_policy::reference_internal)
        .def("grad_sum_const", py::overload_cast<>(&is_t::grad_sum, py::const_),
             py::return_value_policy::reference_internal)
        ...
        .def(py::pickle(
            [](const is_t& p) {  // __getstate__
                ...
                return py::make_tuple(p.type_I_sum(), p.grad_sum(),
                                      p.n_params());
            },
            [](py::tuple t) {  // __setstate__
                ...
                using type_I_sum_t =
                    std::decay_t<decltype(std::declval<is_t>().type_I_sum())>;
                using grad_sum_t =
                    std::decay_t<decltype(std::declval<is_t>().grad_sum())>;
                using n_params_t =
                    std::decay_t<decltype(std::declval<is_t>().n_params())>;
                ...
                is_t p(type_I_sum.rows(), type_I_sum.cols(), n_params);
