...
def to_array(v, l):
    if isinstance(v, float):
        v = [v]
    v = np.array(v*l) if len(v) == 1 else np.array(v)
    if v.shape[0] != l:
        raise ValueError(f"v (={v}) must be either dimension 1 or l (={l}).")
    return v
...
def make_grid(size, lower, upper):
    ...
    theta_grids = (
        Gridder.make_grid(size, lower[i], upper[i])
        for i in range(len(lower))
    )
    # make corresponding radius
    radius = [
        Gridder.radius(size, lower[i], upper[i])
        for i in range(len(lower))
    ]
