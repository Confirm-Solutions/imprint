        ...
        )
{
    ...
    auto run_partition = [&](size_t p_batch_size_) {
        ...
        auto factor = n_sim_per_thr / static_cast<double>(n_sim);
        for (size_t i = 0; i < upper_bds.size()-1; ++i) {
            upper_bd_full.pool(upper_bds[i], factor, n_sim_per_thr);
        }
        factor += n_sim_remain / static_cast<double>(n_sim);
        upper_bd_full.pool(upper_bds.back(), factor, n_sim_per_thr+n_sim_remain);
