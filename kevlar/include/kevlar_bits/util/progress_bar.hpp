...
struct ProgressBar 
{
    ...
    void update(OStreamType& os, size_t amt=1) {
        ...
        double perc = (n_iter_ == n_total_) 
            ? 100. : (n_iter_ * 100.) / n_total_;
        ...
        if (prev_perc_ < 10) os << "\b";
        else if (prev_perc_ >= 10 && prev_perc_ < 100) os << "\b\b";
        else if (prev_perc_ == 100) os << "\b\b\b";
        ...
        if (extra_n_finished > 0) {
            ...
            // delete some remaining chars, add extra finish chars, add back remaining chars
            for (int j = 0; j < bar_length_-n_finished_; ++j) os << '\b';
            for (int j = 0; j < extra_n_finished; ++j) os << finish_char_;
            for (int j = 0; j < bar_length_-new_n_finished; ++j) os << remain_char_;
        ...
        if (show_time_) {
            ...
            size_t dur = std::chrono::duration_cast<
                std::chrono::seconds>(new_tp - tp_).count();
            format_time(dur, os);
...
struct ProgressBarOSWrapper : ProgressBar
{
    ...
    ProgressBarOSWrapper(OStreamType& os)
        : os_(os)
    {}
