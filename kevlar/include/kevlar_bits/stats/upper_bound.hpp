...
struct UpperBound
{
     ...
     *                              Currently, it stores the corner points of each tile
     *                              that maximizes the d1 + d1u + d2u.
     *                              Note that for regular gridpoints, 
     *                              the saved vertices are undefined.
     *                              Default is false.
     */
        ...
        if (verbose) {
            ...
            create_internal(
                    ...
                    [&](size_t m, size_t pos, const auto& v) {
                        Eigen::Map<mat_type<value_t> > 
                            cor_slice(vertices_.data() + slice_size*m,
                                      n_params,
                                      n_tiles);
                        cor_slice.col(pos) = v;
                    });
...
private:
            ...
            SaveCornerType save_corner)
    {
        ...
        value_t d1u_factor = std::sqrt(1./((1.0-delta_prop_0to1)*delta) - 1.);
        ...
        for (size_t gp = 0; gp < n_gridpts; ++gp) {
            ...
            if (grid_range.is_regular(gp)) {
                ...
                delta_0_u_.col(pos) = (d0u_factor / sqrt_ss) * 
            ...
            for (size_t i = 0; i < grid_range.n_tiles(gp); ++i, ++pos) {
                ...
                // set current max value of d1 + d1u + d2u = -inf for all models.
                d11u2u.fill(neg_inf);
