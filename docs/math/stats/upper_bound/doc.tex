...
For example, an exponential model where the control and treatment arms are assumed to be 
...
and $h := \lambda_t / \lambda_c$ is the hazard rate.
...
We wish to construct the upper bound estimate under any parametrization
from a grid space to the natural parameter space,
provided that the mapping is sufficiently smooth.

In the subsequent sections,
we will use the notation $\theta \in \R^s$ to denote 
...
Let $f(\eta) := \PPP_{\eta}\paren{A}$ where $A$ is the event of rejection.
...
\begin{align*}
    ...
    1-\delta_2
\end{align*}
where $T_{t}$ is the sufficient statistic at time $t$
and $\tau_{\max}$ is a (fixed) maximum time.
...
\begin{align*}
    ...
    (T(X_i) - \nabla A(\eta_0)) \indic{X_i \in A}
\end{align*}

Combining the two estimates, we get that for any $\eta \in R$,
\begin{align*}
    &\PPP_{\eta_0}\paren{%
        \sup\limits_{\eta \in R} f(\eta)
        >
        \hat{f}(\eta_0) + \delta_0^u
        +
        \max\limits_{m=1,\ldots, M} \hat{c}_m
    }
    \leq
...
Given a bounded subset of the natural parameter space $H$
...
Next, we bound the hessian term
\begin{align*}
...
\[
    \PPP_{\theta_0}\paren{%
        ...
        +
        \frac{1}{2}\paren{U_1(v_m) + U_2(v_m)}
        \leq 
        \hat{c}_m
    } 
    \geq 1-\delta_2
...
\[
    ...
    1-\delta_2
\]
by convexity of $U_1, U_2$ and that $R$ is a convex hull.
...
where $\widehat{\nabla f}(\theta_0)_i := D_\theta \eta(\theta_0)^\top (T(X_i)-\nabla_\eta A(\eta_0))$.
In that endeavor,
\begin{align*}
