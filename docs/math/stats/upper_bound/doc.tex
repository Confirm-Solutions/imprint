...
For now, assume we have a function $U(v)$ such that
\begin{align*}
...
we will propose ways of finding such a $U$.
Then,
\begin{align*}
...
\[
    ...
    1-\delta_2
\]
as soon as we further assume that $U$ is convex.
...
we will now discuss a way to find the upper bound $U(v)$ to
$\sup\limits_{\theta \in R} v^\top \nabla^2 f(\theta) v$.
In Section~\ref{ssec:higher-order-ub-terms},
we made the additional assumption that $U$ is convex,
...
Note that in general, we can make $U(v)$ even more conservative
