...
def adagrid(lower, upper, model, alpha, init_size, max_iter, N_init, N_max):
    ...
    # This loop is just to get a reasonable estimate for the thresholds.
    ...
    while (not grid_q.empty()) and (itr < max_iter):
        ...
        model.da_dthresh = (alpha_hat - alpha_minus_hat) / (model.thresh - model.thresh_minus)
        model.thresh += (model.alpha_target - alpha_hat) / model.da_dthresh
        model.thresh_minus += (model.alpha_minus_target - alpha_minus_hat) / model.da_dthresh
...
if __name__ == '__main__':
    model = Binomial2Arm()
    grid = adagrid(lower=np.array([-2, -2]),
                   upper=np.array([2, 2]),
                   model=model,
                   alpha=0.025,
                   init_size=8,
